...
function countVowels(str) {
  ...
  return str.split('').reduce((acc, char) => acc + (set.has(char) ? 1 : 0), 0);
}
...
function isPalindrome(str) {
  const clearStrArr = str
    .split('')
    .map((c) => (c.toLowerCase() === c.toUpperCase() ? '' : c.toLowerCase()));
...
function encodeToRot13(str) {
  ...
  str.split('').forEach((c) => {
    const index = origin.indexOf(c);

    result += index === -1 ? c : encoded[index];
  });
